...
export const getWechatUserAccessToken = (
  code: string,
  appId: string,
  secret: string
): Promise<IWechatOpenid> => {
  return new Promise((resolve, reject) => {
    makeHttpRequest<IWechatOpenid>(
      `https://api.weixin.qq.com/sns/oauth2/access_token?appid=${appId}&secret=${secret}&code=${code}&grant_type=authorization_code`,
      {
        dataType: 'json',
      }
    )
      .then(({ data }) => resolve(data as IWechatOpenid))
      .catch(err => reject(err));
  });
};
// refresh_token 有效期30天，access_token 有效期2小时
export const getWechatUserRefeshAccessToken = (
  appId: string,
  refresh_token: string
): Promise<IWechatOpenid> => {
  return new Promise((resolve, reject) => {
...
export const getWechatUserInfo = async (
  ...
  try {
    const { data } = await makeHttpRequest<IWechatInfo>(
...
export const sendMessage = async (info: TInfo, data: IMessage) => {
  ...
  delete myData['url'];
  makeHttpRequest(
    `https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=${info.access_token}`,
    {
      method: 'POST',
      data: {
        // touser 接收者openid
