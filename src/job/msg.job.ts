...
export class DataSyncCheckerJob implements IJob {
  ...
  async onTick() {
    ...
    const members = data.menberUsers.filter(item => {
      const index = sendMembers.find(mem => mem.id === item.id);
      if (index) {
    ...
    const redisMember: TSendMemberNote[] = data.menberUsers.map(item => {
      const one = sendMembers.find(mem => mem.id === item.id);
      if (one) {
        return dayjs(one.sendTime).isBefore(Date.now())
          ? { ...one, sendTime: one.sendTime + 1000 * 60 * 60 * 24 * 2 }
          : one;
      } else {
        return {
          id: item.id,
          openid: item.openid,
          sendTime: Date.now() + 1000 * 60 * 60 * 24 * 2,
        };
      }
    });
    redisMember.length &&
      this.redisService.set('zfxy-send-member', JSON.stringify(redisMember));
    // 重新设定已发送消息的员工member
    ...
    redisNote.length &&
      this.redisService.set('zfxy-send-note', JSON.stringify(redisNote));
    // 重新设定已发送消息的员工note
    // 给员工发送消息
